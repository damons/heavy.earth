/**
  ...
  This is a modified version of Morgan McGuire's Markdeep
  script.  Powerful stuff.  Thank you, McGuire!

  Portions Copyright 2017, Damon Sicore, https://damon.sicore.com
  All rights reserved.
...
var MARKDEEP_FOOTER = '';
var PC_COLOR = '#FFF';
var PC_BACKGROUND_COLOR = '#000';
var PC_FONT = 'Roboto Mono';
var PC_FONT_WEIGHT = '100';
var PC_HEADER_FONT = 'Roboto';
var PC_HEADER_FONT_WEIGHT = '300';
var PC_FONT_MONO = "'Roboto Mono', monospace;";
var PC_FONT_MONO_WEIGHT = "100";
var PC_FONT_CONDENSED = 'Roboto Condensed';
var PC_FONT_CONDENSED_WEIGHT = '300';
...
var codeFontStack = PC_FONT_MONO;
var codeFontSize  = 105.1316178 / measureFontSize(codeFontStack) + 'px';
var codeFontWeight  = '100';

var BODY_STYLESHEET = entag('style', 'body{max-width:72vw;' +
    'margin:auto;' +
    'padding:20px;' +
    'text-align:justify;' +
    'line-height:140%; ' +
    'color:' + PC_COLOR + ';' +
    'background-color:' + PC_BACKGROUND_COLOR + ';' +
    'font-weight:' + PC_FONT_WEIGHT + ';' +
    'font-family:' + PC_FONT + ';}');
...
var STYLESHEET = entag('style',
    ...
    'font-weight:' + PC_FONT_MONO_WEIGHT + ';' +
    'line-height:140%' +
    '}' +
    ...
    'border-bottom:1px solid '+PC_COLOR+';' +
    'color:'+PC_COLOR+';' +
    'font-size:18px;' +
    '}' +
    ...
    '.md .shortTOC{text-align:center;margin-top:15px;font-size:14px}');
    ...
    if (DEBUG_SHOW_SOURCE) {
        ...
        for (var x = 0; x < grid.width; ++x) {
            for (var y = 0; y < grid.height; ++y) {
                var c = grid(x, y);
                if (c !== ' ') {
                    svg += '<text text-anchor="middle" x="' + (x * SCALE) + '" y="' + (4 + y * SCALE * ASPECT) + '" style="fill:#F00;font-family:' + PC_FONT_MONO + '; font-size:12px ;text-align:center">' + escapeHTMLEntities(c) +  '</text>';
                } // if
            } // y
        } // x
        svg += '</g>';
...
var HIGHLIGHT_STYLESHEET =
        "<style>.hljs{display:block;overflow-x:auto;padding:0.5em;background:" + PC_BACKGROUND_COLOR +"; color:" + PC_COLOR + ";-webkit-text-size-adjust:none}"+
        ".hljs-comment{color:"+PC_COLOR+"}" +
        ".hljs-keyword{color:#00F}" +
        ".hljs-literal,.nginx .hljs-title{color:#F00}" +
        ".method,.hljs-list .hljs-title,.hljs-tag .hljs-title,.setting .hljs-value,.hljs-winutils,.tex .hljs-command,.http .hljs-title,.hljs-request,.hljs-status,.hljs-name{color:#00F}" +
        ".hljs-envvar,.tex .hljs-special{color:#FF0}" +
        ".hljs-string{color:#F00}" +
        ...
        ".hljs-function .hljs-title{;color:"+PC_COLOR+"}" +
        ...
        ".hljs-variable{color:"+PC_COLOR+"}" +
        ".css .hljs-tag,.hljs-rule .hljs-property,.hljs-pseudo,.hljs-subst{color:"+PC_COLOR+"}" +
        ".css .hljs-class,.css .hljs-id{color:"+PC_COLOR+"}" +
        ".hljs-value .hljs-important{color:#0FF;}" +
        ".hljs-rule .hljs-keyword{color:#FF0}" +
        ".hljs-annotation,.apache .hljs-sqbracket,.nginx .hljs-built_in{color:#9b859d}" +
        ".hljs-preprocessor,.hljs-preprocessor *,.hljs-pragma{color:#643820}" +
        ".tex .hljs-formula{background-color:"+PC_BACKGROUND_COLOR+";font-style:italic}" +
        ".diff .hljs-header,.hljs-chunk{color:#808080;}" +
        ".diff .hljs-change{background-color:#bccff9}" +
        ".hljs-addition{background-color:#baeeba}" +
        ".hljs-deletion{background-color:#ffc8bd}" +
        ".hljs-comment .hljs-doctag{}" +
        ".method .hljs-id{color:"+PC_COLOR+"}</style>";

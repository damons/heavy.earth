...
SIZE = PANEL['inches']
WIDTH, HEIGHT = PANEL['tileWidthInches'], PANEL['tileHeightInches']
svg = ['<svg xmlns="http://www.w3.org/2000/svg" width="8in" height="8in" viewBox="0 0 8 8">',
       ...
       f'<desc>8 inch square. Diamonds are {WIDTH:g} inch wide and {HEIGHT:g} inch high. Origin is the top-left corner. All panels use this same grid without rotation.</desc>',
...
text(36,719,f'8 x 8 in panel  |  {WIDTH:g} x {HEIGHT:g} in diamonds  |  exact 2:1 slope (26.565 degrees)',9)
...
text(460,32,PANEL['standard'],8,'Courier')
c.showPage()
...
text(36,143,f'Horizontal adjacency: 8 inches = {SIZE/WIDTH:g} horizontal grid repeats.',10)
text(36,125,f'Vertical adjacency: 8 inches = {SIZE/HEIGHT:g} vertical grid repeats.',10)
...
for py in range(n):
    ...
    for px in range(n):
        ...
        # Define the illustration footprint in inches so density changes preserve it.
        a,b=u*WIDTH,v*WIDTH
        floor=(7<=a<19 and 0<=b<11 and (8<=a<11 or 5<=b<8 or (13<=a<18 and 0<=b<3)))
...
for i, name in enumerate(['Western passage','Eastern passage']):
    panels.append({'id':f'example-panel-{i+1}','name':name,'inches':SIZE,'pitch':pitch,'origin':PANEL['origin'],'top':PANEL['top'],
        'assembly':[i,0],'threshold':150,'art':art,'cells':[{'x':int(9/WIDTH),'y':int(6/WIDTH),'kind':'open'}],
        'connectors':[{'id':f'example-passage-{i+1}','x':int(9/WIDTH),'y':int(6/WIDTH),'direction':'east' if i==0 else 'west','target':f'example-passage-{2-i}'}]})
draft={'format':'heavy-earth-panel-draft','version':2,'projection':{'standard':PANEL['standard'],'ratio':'2:1','previewPixels':n,'tileWidthInches':WIDTH},'panels':panels}
